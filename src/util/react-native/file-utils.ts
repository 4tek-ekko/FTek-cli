...
import * as pfs from "../misc/promisfied-fs";
...
export function copyFileToTmpDir(filePath: string): string {
  if (!isDirectory(filePath)) {
    const outputFolderPath: string = temp.mkdirSync("ftek");
...
export async function moveReleaseFilesInTmpFolder(updateContentsPath: string): Promise<string> {
  let tmpUpdateContentsPath: string = temp.mkdirSync("ftek");
  tmpUpdateContentsPath = path.join(tmpUpdateContentsPath, "FTek");
